...
def get_embeddings(texts: List[str], model: str = EMBEDDING_MODEL, num_workers: int = 4) -> List[List[float]]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        future_to_embeddings = {executor.submit(get_single_embedding, text, model): text for text in texts}
        embeddings = []
        max_length = 0
    ...
    embeddings = [np.concatenate([e, np.zeros(max_length - len(e))]) if len(e) != max_length else e for e in embeddings]
    return embeddings
...
def get3questions(sources,embeddings):
  try:
      ...
      n_clusters = max(1, min(n_clusters, len(matrix) - 1))  # Ensure n_clusters is at least 1 and not more than the number of samples
...
def file2embedding(folder, contents=None, batch_size=10, num_workers=4):
  try:
    ...
    source_chunks = [sources[i:i + batch_size] for i in range(0, len(sources), batch_size)]
    # Process the source chunks in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        future_to_embeddings = {executor.submit(get_embeddings, chunk): chunk for chunk in source_chunks}
        embeddings = []
        max_length = 0
        for future in concurrent.futures.as_completed(future_to_embeddings):
            chunk_embeddings = [np.array(e) for e in future.result()]
            # Find the maximum length of the embeddings
            max_length = max(max_length, max([len(e) for e in chunk_embeddings]))
    ...
    with open(folder, 'w', encoding='utf-8') as handle2:
        json.dump({"sources": sources, "embeddings": embeddings_list, "questions": questions}, handle2, ensure_ascii=False, indent=4)
...
def ask(question:str,embeddings,sources):
    ...
    for candi in ordered_candidates:
        ...
        if len(next)>CONTEXT_TOKEN_LIMIT:
            print(f"Exceeded CONTEXT_TOKEN_LIMIT at candidate index {candi[1]}")
            break
        ctx = next
    if len(ctx) == 0:
      return u""    
